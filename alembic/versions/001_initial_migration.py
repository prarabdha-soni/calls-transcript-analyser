...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX idx_transcript_fts ON calls USING gin(to_tsvector('english', transcript))")
    ...
    op.execute("CREATE INDEX idx_transcript_trgm ON calls USING gin(transcript gin_trgm_ops)")
