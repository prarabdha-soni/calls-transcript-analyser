...
from app.schemas import (
    Call, CallListResponse, CallRecommendationsResponse,
    CallRecommendation, CoachingNudge, AgentAnalyticsResponse,
    CallQueryParams, ErrorResponse
)
...
async def get_call_recommendations(
    ...
    for call_obj, similarity in similar_calls:
        ...
        preview = call_obj.transcript[:100] + "..." if len(call_obj.transcript) > 100 else call_obj.transcript
        
        recommendations.append(CallRecommendation(
...
async def get_agent_analytics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving agent analytics: {str(e)}"
        )
