...
async def startup_event():
    ...
    await pool_manager.initialize_pool(
        settings.database_url_async, pool_size=20, max_overflow=30
    )
...
@app.get(
    f"{settings.api_v1_prefix}/calls", response_model=CallListResponse, tags=["Calls"]
)
...
async def get_calls_optimized(
    ...
    max_sentiment: Optional[float] = Query(
        None, ge=-1, le=1, description="Maximum sentiment score"
    ),
...
@app.get(
    f"{settings.api_v1_prefix}/calls/{{call_id}}", response_model=Call, tags=["Calls"]
)
@cache_response(ttl=300)  # Cache for 5 minutes
@monitor_performance
async def get_call_optimized(call_id: str, db: AsyncSession = Depends(get_async_db)):
...
@app.get(
    f"{settings.api_v1_prefix}/calls/{{call_id}}/recommendations",
...
async def get_call_recommendations_optimized(
    call_id: str, db: AsyncSession = Depends(get_async_db)
):
...
@app.get(
    f"{settings.api_v1_prefix}/analytics/agents",
    response_model=AgentAnalyticsResponse,
    tags=["Analytics"],
)
@cache_response(ttl=300)  # Cache for 5 minutes
@monitor_performance
async def get_agent_analytics_optimized(db: AsyncSession = Depends(get_async_db)):
...
@app.post(f"{settings.api_v1_prefix}/calls/bulk", tags=["Calls"])
...
async def get_performance_stats():
    ...
    total_api_calls = sum(metric["count"] for metric in metrics["api_calls"].values())
