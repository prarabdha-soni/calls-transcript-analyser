...
class LoadTester:
    ...
    async def test_api_endpoint(self, endpoint: str, method: str = "GET", 
                               ...
                               total_requests: int = 100) -> LoadTestResult:
        """Test a specific API endpoint under load"""
        print(f"Testing {method} {endpoint} with {concurrent_requests} concurrent requests, {total_requests} total")
    ...
    async def test_data_ingestion_load(self, num_calls: int = 1000, 
                                     batch_size: int = 50) -> LoadTestResult:
        """Test data ingestion performance under load"""
        print(f"Testing data ingestion with {num_calls} calls in batches of {batch_size}")
        ...
        result = LoadTestResult(
            ...
            min_response_time=min(response_times) if response_times else 0,
            max_response_time=max(response_times) if response_times else 0,
            requests_per_second=successful_ingestions / total_time if total_time > 0 else 0,
    ...
    async def test_concurrent_queries(self, num_queries: int = 500, 
                                    concurrent_requests: int = 20) -> LoadTestResult:
        """Test concurrent query performance"""
        print(f"Testing concurrent queries: {num_queries} queries with {concurrent_requests} concurrent")
        ...
        async def make_query_request():
            ...
            try:
                async with self.session.get(f"{self.base_url}/api/v1/calls", params=params) as response:
                    await response.text()
    ...
    def print_results(self):
        ...
        for result in self.results:
            ...
            if result.response_times:
                p95 = sorted(result.response_times)[int(len(result.response_times) * 0.95)]
                p99 = sorted(result.response_times)[int(len(result.response_times) * 0.99)]
...
async def main():
    parser = argparse.ArgumentParser(description="Load test the Sales Call Analytics API")
    parser.add_argument("--base-url", default="http://localhost:8000", help="Base URL of the API")
    parser.add_argument("--ingestion-calls", type=int, default=1000, help="Number of calls to ingest")
    parser.add_argument("--concurrent-queries", type=int, default=500, help="Number of concurrent queries")
    parser.add_argument("--concurrent-requests", type=int, default=20, help="Number of concurrent requests")
    ...
    async with LoadTester(args.base_url) as tester:
        # Test API endpoints
        await tester.test_api_endpoint("/api/v1/calls", concurrent_requests=10, total_requests=100)
        await tester.test_api_endpoint("/api/v1/analytics/agents", concurrent_requests=5, total_requests=50)
        
        # Test data ingestion
        await tester.test_data_ingestion_load(args.ingestion_calls, batch_size=50)
        
        # Test concurrent queries
        await tester.test_concurrent_queries(args.concurrent_queries, args.concurrent_requests)
