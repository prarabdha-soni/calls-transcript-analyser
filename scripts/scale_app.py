...
class AppScaler:
        ...
        for i in range(self.num_instances):
            ...
            process = subprocess.Popen(
                [
                    ...
                    str(port),
                    "--workers",
                    "1",  # Single worker per instance for now
                    "--log-level",
                    "info",
                ]
            )
    ...
    def get_instance_urls(self) -> List[str]:
        ...
        return [
            f"http://localhost:{self.base_port + i}" for i in range(self.num_instances)
        ]
    ...
    async def run_load_test(self, target_url: str = None, duration: int = 60):
        ...
        async with LoadTester(target_url) as tester:
            ...
            await tester.test_api_endpoint(
                "/api/v1/calls", concurrent_requests=50, total_requests=500
            )
            await tester.test_concurrent_queries(
                num_queries=1000, concurrent_requests=50
            )
    ...
    def monitor_resources(self):
        ...
        for i, process in enumerate(self.processes):
            try:
                ...
                logger.info(
                    f"Instance {i+1} - CPU: {proc_info.cpu_percent()}%, Memory: {proc_info.memory_info().rss // 1024 // 1024}MB"
                )
...
async def main():
    ...
    parser.add_argument(
        "--base-port", type=int, default=8000, help="Base port for instances"
    )
